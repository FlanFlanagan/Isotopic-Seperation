...
stg = 1
a = MassStream('CooledIsos.txt', 1)
for key in a.comp.keys():
    ##################
    ### Prep Work! ###
    ##################
    ...
    rm1 = [i]
    if stg == 1:
      LWR_Cooled = remover.remove(LWR_Cooled, rm1)
    ...
    LWR_stream = LWR_RepOut.mult_by_mass()
    ...
    #################################
    if stg == 2:
      FR_Stor = remover.remove(LWR_Cooled, rm1)
