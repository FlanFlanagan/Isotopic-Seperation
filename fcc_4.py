...
U235 = Material({922350: 1.0}, 0.04, "U235")
U238 = Material({922380: 1.0}, 0.96, "U238")
...
LWR.calc()
LWR.write()
LWR_SNF = 1.0 * LWR.mat_prod
LWR_SNF.name = "LWR_SNF"

LWR_Stor.decay_time = LWR_SNF_Storage_Time
LWR_Cooled = LWR_Stor.calc(LWR_SNF)
...
for isos1 in RmIsos:
    ...
    RmvIsos = [isos1]
    RmStor[isos1] = LWR_Cooled.comp[isos1]
    stor_t = 0 * half_life(isos1)
    Rm_Stor = Rmv_Stor.calc(RmStor, stor_t)
    LWR_Cooled = remove(LWR_Cooled, RmvIsos)
    LWR_Cooled.mass = (mass_con)-(mass_con*RmStor[isos1])
...
#   pnl_a  = 0.1
    ...
    DoA = 10.0**(-3)        #Degree of accuracy to carry out calculations to.
    q = 0
...
RmStor = {}
mass_con_2 = StorOut.mass
for isos2 in RmIsos:
    if isos2 in StorOut.comp.keys():
	RmStor[isos2] = StorOut.comp[isos2]
	StorOut = remove(StorOut, [isos2])
	StorOut.mass = (mass_con_2)-(mass_con_2*RmStor[isos2])
	mass_con_2 = StorOut.mass 
Rm_Stor = Rmv_Stor.calc(RmStor, FR_SNF_Storage_Time)
    #print(Rm_Stor.mass)
Rmv_Stor.write()
FR_Stor.write()
...
disos = os.getcwd()
os.chdir("/home/robert/Desktop/Fuel Cycle Runs/Sep. Eff. = 0.99/fcc_4")
writer3 = open('BUd_.txt', 'a')
writer3.write(str(RmIsos)+'_'+str(LWR.BUd*snf_need[-1])+'_'+str(FR.BUd)+'\n')
os.chdir(disos)

os.chdir("/home/robert/Desktop/Fuel Cycle Runs/Sep. Eff. = 0.99/fcc_4")
writer3 = open('Mass.txt', 'a')
writer3.write(str(RmIsos)+'_'+str(LWR_CoreInput.mass*snf_need[-1])+'_'+str(CoreInput.mass)+'\n')
os.chdir(disos)
